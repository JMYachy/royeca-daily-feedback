...
async function loadData() {
  ...
  if (!supabase && !initSupabase()) {
    renderStatus("Supabase Error", "Supabase could not be initialized.");
    return;
  }
  ...
  if (error) {
    ...
    renderStatus("Query Error", error.message);
    return;
  }
...
function filterByPeriod(rows) {
  ...
  return rows.filter(r =>
    parseDate(r.created_at).getTime() >= start
  );
}
...
function updateDeptChip() {
  ...
  document.getElementById("d-idx").textContent =
    `${deptIdx + 1} / ${DEPTS.length}`;
...
function render(slide = false) {
  ...
  const forDept =
    inWindow.filter(r => r.branch_name === dept.id);
...
function buildStrip(emp, cli) {
  ...
  const avg = total
    ? (all.reduce((s, r) => s + Number(r.rating), 0) / total).toFixed(1)
    : "—";

  const eAvg = emp.length
    ? (emp.reduce((s, r) => s + Number(r.rating), 0) / emp.length).toFixed(1)
    : "—";

  const cAvg = cli.length
    ? (cli.reduce((s, r) => s + Number(r.rating), 0) / cli.length).toFixed(1)
    : "—";
...
function buildPanels(emp, cli) {
  ...
  wrap.appendChild(
    buildPanel("emp", "👤 Employee", emp, "col-green")
  );

  wrap.appendChild(
    buildPanel("cli", "🤝 Client", cli, "col-gold")
  );
...
function buildPanel(type, label, rows, numColorClass) {
  ...
  const sorted =
    rows.map(r => Number(r.rating)).sort((a, b) => a - b);
  ...
  panel.innerHTML = `
    <div class="panel-head">
      ...
      <div class="mini-stats">
        ...
        <div class="mini-stat">
          <div class="mini-stat-n ${numColorClass}">
            ${avg}
          </div>
  ...
  if (total === 0) {
    ...
    freq.forEach((count, i) => {
      ...
      col.innerHTML = `
        <div class="bar-track"
             style="height:${trackH}%;background:rgba(155,236,0,0.05);">

          <div class="bar-fill ${FC[i]}${isDom ? " dom" : ""}"
               style="height:0%;"></div>

          <div class="bar-tip">
            ${count}× ${EMOJIS[i]} · ${score}/10
          </div>
